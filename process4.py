...
class Computation:
    ...
    def produce_result(self, system_state):
        s = 0
        ...
        value = self.c + sum(system_state) + os.getpid()
        return value
...
if __name__ == '__main__':
    ...
    total_data = [system_state for _ in range(n_reactions)]   # List of lists
    ...
    result = p.map(obj.produce_result, total_data)

    print(result)
    print(f"Finished in {(time.perf_counter() - t_start):.2f} sec")


    system_state = [11,12,13,14]
    total_data = [system_state for _ in range(n_reactions)]   # List of lists
    # map list to target function
    result = p.map(obj.produce_result, total_data)
