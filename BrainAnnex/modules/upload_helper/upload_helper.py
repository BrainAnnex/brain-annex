...
class UploadHelper:
    ...
    @classmethod
    def store_uploaded_file(cls, request_obj, upload_dir: str, key_name=None, verbose=False) -> (str, str):
        """
        ...
        :param request_obj: A flask.request object
        :param upload_dir:  Name of temporary directory to use for the upload.  EXAMPLE on Windows: "D:/tmp/"
        :param key_name:    (Optional) The name that was used in the HTML form INPUT tag:
        ...
        if len(files_multi_dict) == 0:
            raise Exception(f"No files found in upload!  (The ImmutableMultiDict in request.files is empty)")
...
class ImageProcessing:
    ...
    @classmethod
    def process_uploaded_image(cls, filename: str, fullname: str, media_folder: str):
        """
        Obtain the size of the image, resize it to a thumbnail,
        save it in the specified media folder,
