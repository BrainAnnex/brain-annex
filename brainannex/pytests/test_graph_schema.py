...
def test_create_class(db):
    ...
    assert result == [{'name': 'French Vocabulary', 'uri': french_class_uri, 'strict': False, '_internal_id': french_class_id}]

    class_A_id = GraphSchema.create_class("A", strict=True)
    class_A_uri = GraphSchema.get_class_entity_id(class_name="A")
    result = db.get_nodes(match_specs, return_internal_id=True)
    expected = [{'name': 'French Vocabulary', 'uri': french_class_uri, 'strict': False, '_internal_id': french_class_id},
                {'name': 'A', 'uri': class_A_uri, 'strict': True, '_internal_id': class_A_id}]
...
def test_create_class_relationship(db):
    ...
    course_id = GraphSchema.create_class("Course")
...
def test_get_class_properties(db):
    ...
    neo_uri= GraphSchema.create_class("My BIG class")
    props = GraphSchema.get_class_properties(neo_uri)
    assert props == []
...
def test_is_property_allowed(db):
    ...
    GraphSchema.create_class_with_properties("Content Item", ["uri"], strict=True)
...
def test_get_nodes_by_filter(db):
    ...
    assert result == ( [{'_CLASS': 'Car', 'color': 'grey', 'make': 'Toyota'},
                     ...
                     ] , 4 )          #  The record with no date will be at the end, when sorting in descending order
    ...
    assert result == ( [{'make': 'Chevrolet', 'year': 1955, '_CLASS': 'Car', 'color': 'pink'},
                        ...
                        {'color': 'yellow', 'year': 1999},
                        {'color': 'black', 'trim': 'yellow', 'year': 1999},
                        {'make': 'Toyota', '_CLASS': 'Car', 'color': 'grey'}] , 7 )
...
def test_create_data_node_3(db):
    ...
    assert len(result) == 1
    assert result == [[{'_internal_id': new_datanode_internal_id, '_node_labels': ['Car'], '_CLASS': "Car"}]]   # No other properties were set
    ...
    result = db.query_extended(q)
    assert len(result) == 1
    assert result == [[{'_internal_id': new_datanode_internal_id, '_node_labels': ['Car'], 'color': 'green', 'year': 2022, '_CLASS': "Car"}]]
...
def test_add_data_node_merge(db):
    ...
    result = db.query_extended(q)
    assert len(result) == 1
    assert result == [[{'color': 'white', '_internal_id': new_datanode_id, '_node_labels': ['Car'], '_CLASS': "Car"}]]
    ...
    result = db.query_extended(q)
    assert len(result) == 1
    assert result == [[{'color': 'white', '_internal_id': new_datanode_id, '_node_labels': ['Car'], '_CLASS': "Car"}]]   # Same as before
    ...
    result = db.query_extended(q)
    assert len(result) == 1
    assert result == [[{'color': 'red', '_internal_id': new_datanode_id, '_node_labels': ['Car'], '_CLASS': "Car"}]]
    ...
    result = db.query_extended(q)
    assert len(result) == 1
    assert result == [[{'color': 'blue', 'year': 2023, '_internal_id': new_datanode_id, '_node_labels': ['Car'], '_CLASS': "Car"}]]
    ...
    result = db.query_extended(q)
    assert len(result) == 1
    assert result == [[{'color': 'blue', 'year': 2000, '_internal_id': new_datanode_id, '_node_labels': ['Car'], '_CLASS': "Car"}]]
    ...
    result = db.query_extended(q)
    assert len(result) == 1
    assert result == [[{'color': 'red', 'year': 1999, '_internal_id': new_datanode_id, '_node_labels': ['Car'], '_CLASS': "Car"}]]
    ...
    result = db.query_extended(q)
    assert len(result) == 1
    assert result == [[{'color': 'red', 'year': 1999, 'make': 'Toyota', '_internal_id': new_datanode_id, '_node_labels': ['Car'], '_CLASS': "Car"}]]
...
def test_class_of_data_point(db):
    ...
    uri = GraphSchema.reserve_next_entity_id()      # Obtain (and reserve) the next auto-increment value
    GraphSchema.create_data_node(class_name="Person", new_entity_id=uri)
