...
def test_extract_unique_good_words():
    ...
    # Explore partially-numeric values that should be indexed
    text = "TRV240 test8 A5X"
    assert FullTextIndexing.extract_unique_good_words(text) == {'trv240', 'test8', 'a5x'}
...
def test_new_indexing(db):
    ...
    assert NeoSchema.count_data_nodes_of_class(data_node="Word") == 3
    assert NeoSchema.count_data_nodes_of_class(data_node="Indexer") == 1
    assert NeoSchema.count_data_nodes_of_class(data_node="Content Item") == 1
    ...
    assert NeoSchema.count_data_nodes_of_class(data_node="Word") == 4    # One word from earlier ("research") got re-used
    assert NeoSchema.count_data_nodes_of_class(data_node="Indexer") == 2
    assert NeoSchema.count_data_nodes_of_class(data_node="Content Item") == 2
...
def test_remove_indexing(db):
    ...
    assert NeoSchema.count_data_nodes_of_class(data_node="Indexer") == 0 # The Indexer node is gone
